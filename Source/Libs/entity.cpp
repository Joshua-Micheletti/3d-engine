...
/* VERTICES */
/* -----------------------------------------------------------------------------------------------------------------------*/
void Entity::load3DModel(char *model) {
  loadModel(model);
  placeAtCenter();
...
void Entity::loadModel(char *name) {
  FILE *model = fopen(name, "r");
  char buffer[255];
  ...
  for (int i = 0; i < faces.size(); i++) {
    this->vertices.push_back(vertices[(faces[i] - 1) * 3    ]);
    this->vertices.push_back(vertices[(faces[i] - 1) * 3 + 1]);
    this->vertices.push_back(vertices[(faces[i] - 1) * 3 + 2]);
    this->uvs.push_back( tex[(facesTex[i] - 1) * 2    ]);
    this->uvs.push_back(-tex[(facesTex[i] - 1) * 2 + 1]);
    this->normals.push_back(normals[(facesNormals[i] - 1) * 3    ]);
    this->normals.push_back(normals[(facesNormals[i] - 1) * 3 + 1]);
    this->normals.push_back(normals[(facesNormals[i] - 1) * 3 + 2]);
  }
}
