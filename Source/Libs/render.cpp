...
void attachUniforms(Entity *entity, std::vector<uniform_t> uniformBuffer) {
  for (int i = 0; i < uniformBuffer.size(); i++) {
    if (strcmp(uniformBuffer[i].name, "modelMatrix") == 0) {
      ...
      if (strcmp(entity->getName(), "skybox") == 0) {
...
void renderEntities() {
  for (int i = 0; i < entityBuffer.size(); i++) {
    ...
    if (strcmp(entityBuffer[i]->getName(), "skybox") == 0) {
...
void setupRender() {
  ...
  screenShader = new Shader("screen shader");
  screenShader->loadShader("Shader/screen/vertex.c", "Shader/screen/fragment.c");
}
